-- Build industry workshops on z94 using constructWithItems
local z = 94

-- Find boulders for construction
...
if stone then
    local smelter = dfhack.buildings.allocInstance(
        {x=85, y=91, z=z},
        df.building_type.Furnace, df.furnace_type.Smelter, -1)
    if smelter then
        smelter.x1 = 84; smelter.x2 = 86
        smelter.y1 = 90; smelter.y2 = 92
        smelter.centerx = 85; smelter.centery = 91
        if dfhack.buildings.constructWithItems(smelter, {stone}) then
            print("Smelter placed at (84,90,z94)")
        else
            print("Failed to construct smelter")
        end
    end
else
    print("No boulders for smelter")
end

-- Build Metalsmith's Forge (3x3)
stone = get_boulder()
if stone then
    local forge = dfhack.buildings.allocInstance(
        {x=85, y=95, z=z},
        df.building_type.Furnace, df.furnace_type.MetalsmithsForge, -1)
    if forge then
